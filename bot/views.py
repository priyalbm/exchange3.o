...
from .serializers import (
    ...
    WalletBalanceRequestSerializer, CancelOrderSerializer, OrderCheckSerializer
)
...
#                 # Make sure the bot belongs to the authenticated user
#                 bot_config = BotConfiguration.objects.get(id=bot_id, user=request.user)
#                 logs = BotLog.objects.filter(bot_configuration=bot_config).order_by('-created_at')[:limit]
...
#                 )
#         else:
#             # Get logs for all bots belonging to the user
#             user_bots = BotConfiguration.objects.filter(user=request.user).values_list('id', flat=True)
#             logs = BotLog.objects.filter(bot_configuration__in=user_bots).order_by('-created_at')[:limit]
#             serializer = BotLogSerializer(logs, many=True)
#             return Response(serializer.data)
